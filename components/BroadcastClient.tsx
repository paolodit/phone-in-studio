...
export function BroadcastClient({ initialSnapshot, token, mode, layout }: { initialSnapshot: BroadcastSnapshot; token?: string; mode: "full" | "overlay"; layout: BroadcastLayout }) {
  ...
  useEffect(() => {
    const measure = () => setNarrowPane(window.innerWidth / Math.max(window.innerHeight, 1) < 0.82);
    measure();
  ...
  const portrait = !transparent && (layout === "tiktok" || (layout === "web" && narrowPane));
  ...
  return <main className={`h-screen min-h-[420px] overflow-hidden ${transparent ? "bg-transparent" : "bg-[#09101c]"}`}>
    <section className={`relative flex h-full flex-col overflow-hidden ${transparent ? "" : "bg-[radial-gradient(circle_at_75%_15%,#173c5b,transparent_34%),linear-gradient(135deg,#07111e,#101229)]"}`}>
      ...
      {topicVisual && <div className={portrait ? "absolute inset-x-0 top-0 h-[46%]" : "absolute inset-y-0 right-0 w-[43%]"}>
        <img className="h-full w-full object-cover" src={topicVisual.url} alt="" />
        <div className={`absolute inset-0 ${portrait ? "bg-gradient-to-b from-[#09101c]/65 via-transparent to-[#09101c]" : "bg-gradient-to-r from-[#09101c] via-[#09101c]/25 to-transparent"}`} />
        {topicVisual.creditText && <a href={topicVisual.creditUrl} target="_blank" rel="noreferrer" className="absolute bottom-2 right-2 max-w-[70%] truncate bg-black/40 px-1.5 py-0.5 text-[clamp(6px,.55vw,9px)] font-medium text-white/55 backdrop-blur-sm">{topicVisual.creditText}</a>}
      </div>}

      <header className={`relative z-10 flex shrink-0 items-center justify-between gap-4 ${transparent ? "p-6 md:p-8" : portrait ? "px-[clamp(1rem,5vw,2rem)] pt-[clamp(1rem,3vh,2rem)]" : "px-[clamp(1.5rem,4vw,4rem)] pt-[clamp(1.5rem,4vh,3.5rem)]"}`}>
        <div className="min-w-0"><p className="text-[10px] font-black tracking-[.28em] text-cyan-300 md:text-xs">LIVE PHONE-IN</p><h1 className={`mt-1 truncate font-black tracking-tight text-white ${portrait ? "text-[clamp(1.25rem,6vw,2.2rem)]" : "text-[clamp(1.6rem,4vw,3.4rem)]"}`}>{snapshot.title}</h1></div>
      ...
      <div className={`relative z-10 flex min-h-0 flex-1 items-end ${portrait ? "px-[clamp(.9rem,4vw,1.8rem)] pb-[clamp(1rem,4vh,2.5rem)] pt-[30vh]" : `px-[clamp(1.5rem,4vw,4rem)] pb-[clamp(1.5rem,5vh,4rem)] pt-6 ${topicVisual ? "pr-[clamp(1.5rem,42vw,38rem)]" : ""}`}`}>
        {showCaller && caller ? <div className={`grid w-full items-end ${portrait ? "mx-auto max-w-2xl grid-cols-[minmax(76px,24vw)_1fr] gap-[clamp(.65rem,3vw,1.2rem)]" : `gap-[clamp(1rem,2.5vw,2.2rem)] ${comingUp ? "mx-auto max-w-4xl grid-cols-[minmax(110px,180px)_1fr]" : "max-w-6xl grid-cols-[minmax(180px,.72fr)_1.45fr]"} max-[700px]:grid-cols-[minmax(100px,32vw)_1fr]`}`}>
          <div className={`relative aspect-square overflow-hidden rounded-[clamp(.8rem,3vw,2rem)] border-[3px] border-white/20 bg-gradient-to-br from-cyan-300 via-violet-500 to-rose-500 shadow-2xl shadow-black/30 ${comingUp || portrait ? "max-w-[180px]" : ""}`}>
          ...
          <div className={`min-w-0 rounded-[clamp(.8rem,3vw,2rem)] border border-white/15 bg-slate-950/82 shadow-2xl shadow-black/20 backdrop-blur ${portrait ? "p-[clamp(.85rem,3vw,1.4rem)]" : comingUp ? "p-[clamp(1rem,2.5vw,2rem)]" : "p-[clamp(1.25rem,3vw,2.5rem)]"}`}>
            <p className="text-[10px] font-black uppercase tracking-[.2em] text-cyan-300 md:text-sm">{callerKicker}</p>
            <h2 className={`mt-2 font-black tracking-tight text-cyan-200 ${portrait ? "text-[clamp(1.35rem,7vw,2.5rem)]" : comingUp ? "text-[clamp(1.7rem,4vw,3rem)]" : "text-[clamp(2rem,5vw,4.2rem)]"}`}>{caller.name}</h2>
            ...
            <p className={`max-w-3xl font-bold leading-tight text-slate-100 ${portrait ? "mt-3 text-[clamp(.9rem,4vw,1.45rem)]" : comingUp ? "mt-4 text-[clamp(1rem,2.3vw,1.7rem)]" : "mt-[clamp(1rem,3vh,2rem)] text-[clamp(1.25rem,3vw,2.5rem)]"}`}>{caller.issueHeadline}</p>
            <p className={`max-w-2xl leading-relaxed text-slate-300 ${portrait ? "mt-2 line-clamp-3 text-[clamp(.68rem,2.7vw,.92rem)]" : comingUp ? "mt-2 hidden text-sm min-[850px]:block" : "mt-3 text-[clamp(.75rem,1.4vw,1.05rem)]"}`}>{caller.openingSummary}</p>
          </div>
