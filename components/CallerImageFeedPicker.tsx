...
export function CallerImageFeedPicker({ defaultQuery, addVisual }: { defaultQuery: string; addVisual: (formData: FormData) => void | Promise<void> }) {
  ...
  return <section className="panel panel-pad mt-6">
    ...
    <p className="mt-2 text-sm text-slate-400">The image only appears on the broadcast when the host clicks it in Prepared visuals; it never replaces the caller identity card.</p>
    ...
    {results.length > 0 && <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{results.map((image) => <article key={image.id} className="overflow-hidden rounded-xl border border-slate-700 bg-slate-950"><img className="h-36 w-full object-cover" src={image.previewUrl} alt={image.alt} /><div className="p-3"><p className="line-clamp-2 text-sm font-semibold text-white">{image.alt}</p><a href={image.sourceUrl} target="_blank" rel="noreferrer" className="mt-1 block text-xs text-cyan-300 hover:underline">{image.provider} · {image.creator}</a><form action={addVisual} className="mt-3"><input type="hidden" name="label" value={`${image.provider === "pexels" ? "Pexels" : "Pixabay"}: ${image.alt}`} /><input type="hidden" name="url" value={image.imageUrl} /><input type="hidden" name="trigger" value={`Topic image supplied by ${image.provider} (${image.creator}).`} /><button className="button-secondary w-full text-xs">Add to image feed</button></form></div></article>)}</div>}
  </section>;
}
