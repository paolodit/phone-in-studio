...
export function CallerIdeaLab() {
  ...
  function clearGenerated() {
    setPremises([]); setSelectedPremise(null); setDraft(null); setSavedDraft(null); setNotice(null); setShowOptions(true);
  }
  function updateSource(value: string) { setSourceNotes(value); clearGenerated(); }
  ...
  function toggleDictation() {
    ...
    recognition.onresult = (event) => {
      ...
      if (spoken) { setSourceNotes((current) => [current.trim(), spoken].filter(Boolean).join(" ")); clearGenerated(); }
    };
    ...
    setDictating(true);
    try { recognition.start(); } catch { setDictating(false); setError("Dictation could not start. Please try again or type your note."); }
  }
  ...
  return <div className="space-y-6">
    <ol aria-label="Caller building steps" className="flex flex-wrap gap-4 text-xs font-semibold">{["Describe", "Choose a direction", "Review & save"].map((step, index) => <li key={step} className={(draft ? 2 : premises.length ? 1 : 0) === index ? "text-cyan-200" : "text-slate-500"}><span className="mr-2 inline-grid h-5 w-5 place-items-center rounded-full border border-current">{index + 1}</span>{step}</li>)}</ol>
    <fieldset disabled={busy !== null} className="panel panel-pad min-w-0">
      ...
      <div className="mt-5 flex items-end justify-between gap-3"><label htmlFor="caller-seed" className="label">Seed / source notes</label><button className="button-secondary !min-h-8 !px-3 text-xs" type="button" onClick={toggleDictation}>{dictating ? <><MicOff className="h-3.5 w-3.5" /> Stop dictation</> : <><Mic className="h-3.5 w-3.5" /> Dictate seed</>}</button></div><textarea id="caller-seed" className="field mt-1 min-h-36" value={sourceNotes} onChange={(event) => updateSource(event.target.value)} placeholder="For example: a caller believes a neighbourhood book-swap shelf is enforcing a bizarre hierarchy, but they have been quietly replacing the books they dislike." />
      <div className="mt-3 grid gap-3 md:grid-cols-2">
        <label><span className="label">Kind of call <span className="normal-case tracking-normal text-slate-500">(optional)</span></span><select className="field" value={callType} onChange={(event) => { setCallType(event.target.value); clearGenerated(); }}><option value="auto">Surprise me with a mix</option><option value="advice">Advice</option><option value="opinion">Opinion or dispute</option><option value="personal">Personal story</option><option value="practical">Practical problem</option><option value="wildcard">Wildcard</option></select></label>
        <label><span className="label">Tone <span className="normal-case tracking-normal text-slate-500">(optional)</span></span><select className="field" value={tone} onChange={(event) => { setTone(event.target.value); clearGenerated(); }}><option value="auto">Let the idea decide</option><option value="grounded">Grounded</option><option value="lively">Lively</option><option value="reflective">Reflective</option><option value="strange">Strange</option></select></label>
      </div>
      <div className="mt-3 flex flex-wrap gap-2 text-xs"><button className="rounded-full border border-slate-700 px-3 py-1 text-slate-300 hover:border-cyan-300 hover:text-cyan-100" type="button" onClick={() => updateSource("I accepted a promotion over a close friend and now I am not sure whether I am guilty, proud, or both.")}>Work dilemma</button><button className="rounded-full border border-slate-700 px-3 py-1 text-slate-300 hover:border-cyan-300 hover:text-cyan-100" type="button" onClick={() => updateSource("Were bees just lazy bums before pollination was invented?")}>Strange question</button><button className="rounded-full border border-slate-700 px-3 py-1 text-slate-300 hover:border-cyan-300 hover:text-cyan-100" type="button" onClick={() => updateSource("I keep using my old housemate's Netflix profile because deleting it would mean admitting the friendship is over.")}>Personal story</button></div>
      <div className="mt-4 flex flex-wrap items-center gap-3"><button className="button-primary" type="button" onClick={generatePremises} disabled={busy !== null || dictating || sourceNotes.trim().length < 12}>{busy === "premises" ? "Finding distinct callers..." : "Create six caller options"}</button><span className="text-xs text-slate-500">Nothing is saved until you choose a caller.</span></div>
    </fieldset>
