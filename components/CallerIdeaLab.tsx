...
async function postJson<T>(url: string, body: unknown): Promise<T> {
  ...
  if (!response.ok) {
    const message = payload && typeof payload === "object" && "error" in payload && typeof payload.error === "string" ? payload.error : "Something went wrong. Please try again.";
...
export function CallerIdeaLab() {
  ...
  async function saveDraft() {
    if (!selectedPremise || !draft) return;
    setBusy("save");
    setError(null);
    try {
      const result = await postJson<{ callerId: string }>("/api/caller-workshop/save", { sourceNotes, premise: selectedPremise, draft });
      window.location.assign(`/callers/${result.callerId}`);
    } catch (requestError) {
  ...
  return (
    <div className="space-y-6">
      <section className="panel panel-pad">
        ...
        <p className="mt-2 max-w-3xl text-sm text-slate-300">Describe a situation, observation, prop, local texture, or comic disagreement. The workshop will make fictional adult callers only; it does not use the seed as a live script.</p>
        <label className="mt-5 block"><span className="label">Seed / source notes</span><textarea className="field min-h-36" value={sourceNotes} onChange={(event) => { setSourceNotes(event.target.value); setPremises([]); setSelectedPremise(null); setDraft(null); }} placeholder="For example: a caller believes a neighbourhood book-swap shelf is enforcing a bizarre hierarchy, but they have been quietly replacing the books they dislike." /></label>
        <div className="mt-4 flex flex-wrap items-center gap-3"><button className="button-primary" type="button" onClick={generatePremises} disabled={busy !== null || sourceNotes.trim().length < 12}>{busy === "premises" ? "Finding angles…" : "Generate six premise options"}</button><span className="text-xs text-slate-500">OpenAI is called only when you press this button. Nothing is saved yet.</span></div>
      </section>
      ...
      {draft && <section className="space-y-4"><div><p className="eyebrow">Step 3 — Draft review</p><h2 className="mt-1 text-xl font-bold text-white">{draft.firstName} {draft.surnameInitial} — {draft.issueHeadline}</h2><p className="mt-2 max-w-3xl text-sm text-slate-300">This is a private, unapproved production draft. Save it to the normal caller editor to revise it; approval remains a separate deliberate action.</p></div>
        ...
        <button className="button-primary" type="button" onClick={saveDraft} disabled={busy !== null}>{busy === "save" ? "Saving draft…" : "Save as draft and edit"}</button>
      </section>}
    </div>
  );
}
