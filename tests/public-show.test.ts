...
describe("broadcast privacy boundary", () => {
  it("allow-lists public fields and excludes private caller material", () => {
    const result = publicCallerFromSnapshot({
      ...
      assets: [{ id: "asset-1", type: "PORTRAIT", label: "Mandy", url: "/mandy.svg", priority: 0 }, { id: "asset-2", type: "SUPPORTING_VISUAL", label: "Fridge screen", url: "/fridge.svg", creditText: "Alex Example · Pexels", creditUrl: "https://www.pexels.com/photo/1", priority: 1 }],
    }, "asset-2");
    const serialized = JSON.stringify(result);
    expect(result).toMatchObject({ name: "Mandy", location: "Bridgend", issueHeadline: "The fridge is biased" });
    expect(result?.visual).toEqual({ label: "Fridge screen", url: "/fridge.svg", creditText: "Alex Example · Pexels", creditUrl: "https://www.pexels.com/photo/1" });
