...
export default async function OutputPreviewPage({ params }: { params: Promise<{ showId: string }> }) {
  ...
  let data;
  try { data = await Promise.all([getBroadcastSnapshot(showId), prisma.show.findUniqueOrThrow({ where: { id: showId }, select: { broadcastToken: true } })]); }
  catch (error) {
    if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === "P2025") redirect("/shows");
    throw error;
  }
  const [snapshot, show] = data;
