...
export default async function AiHostProfilesPage({ searchParams }: { searchParams: Promise<{ profile?: string; saved?: string }> }) {
  ...
  return <main className="shell"><StudioNav />
    ...
    <div className="mt-5 grid gap-5 xl:grid-cols-[minmax(0,1fr)_minmax(360px,.8fr)]">
      <form key={query.profile === "new" ? "new" : selected?.id} className="panel panel-pad" action={saveHostProfileAction.bind(null, query.profile === "new" ? null : selected?.id ?? null)}>
      ...
      {selected && query.profile !== "new" ? <div className="space-y-4"><HostSoundcheck key={selected.id} profileId={selected.id} /><Link className="button-primary" href="/shows">Choose a show to host →</Link><p className="text-xs leading-5 text-slate-400">In the show’s Options, choose an AI hosting mode and this presenter, then Save & open Studio.</p></div> : <section className="panel panel-pad"><p className="eyebrow">Soundcheck</p><p className="mt-3 text-sm leading-6 text-slate-400">Save this presenter first. A private conversational voice test will then appear here.</p></section>}
    </div>
  </main>;
}
